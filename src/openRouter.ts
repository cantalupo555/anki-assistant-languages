...
function extractTextContent(choices: any[]): string {
    ...
    // Remove the content within the <brainstorming>, <definition_analysis> and <translation_analysis> tags
    let text = textContent.message.content
        ...
        .replace(/<translation_analysis>.*?<\/translation_analysis>/s, '');
    return text;
}
...
// Function to get a short dialogue using a given word or idiomatic expression using OpenRouter
export async function getDialogueOpenRouter(word: string, targetLanguage: string, nativeLanguage: string, llm: string): Promise<[string, TokenCount]> {
    // Construct the prompt for the OpenRouter API
...
Remember to create a dialogue that sounds natural and contextually appropriate. Vary the topics and situations to make the conversation interesting. If the given word or expression is versatile, try to showcase different ways it can be used.`;
    ...
    const dialogue = extractTextContent(response.data.choices);
    // Calculate the token count
    const tokenCount: TokenCount = {
        inputTokens: response.data.usage.prompt_tokens,
        outputTokens: response.data.usage.completion_tokens,
